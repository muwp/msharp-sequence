 ...
 */
public class SnowflakeHelper implements Sequence {
    ...
    @Override
    public synchronized Long nextId(String bizName) {
        long timestamp = timeGen();

        //如果当前时间小于上一次ID生成的时间戳，说明系统时钟回退过这个时候应当抛出异常
        if (timestamp < lastTimestamp) {
            throw new SequenceException("[SnowflakeHelper-nextValue] 当前时间小于上次生成序列号的时间，时间被回退了，请确认服务器时间的设置.");
        }
    ...
    public void setWorkerId(long workerId) {
        if (workerId > maxWorkerId) {
            throw new SequenceException("[SnowflakeHelper-setWorkerId] workerId 不能大于31.");
        }
    ...
    public void setDatacenterId(long datacenterId) {
        if (datacenterId > maxDatacenterId) {
            throw new SequenceException("[SnowflakeHelper-setDatacenterId] datacenterId 不能大于31.");
        }
